...
class _StudentAttendanceScreenState extends State<StudentAttendanceScreen> {
  ...
  void initState() {
    ...
    _loadSessions();
    _refreshTimer =
        Timer.periodic(const Duration(seconds: 15), (_) => _loadSessions());
  }
                      ...
                      : RefreshIndicator(
                          ...
                          child: ListView.builder(
                            padding: const EdgeInsets.fromLTRB(16, 16, 16,
                                100), // Thêm padding 100px ở dưới đáy để không bị thanh Tab Bar lơ lửng che mất
        ...
        child: Row(children: [
          ...
          Expanded(
              child: Column(
                  ...
                  children: [
                const Text('Phiên Điểm Danh',
                    style: TextStyle(
                        color: AppTheme.textPrimary,
                        fontSize: 18,
                        fontWeight: FontWeight.bold)),
                const Text('Chọn lớp đang mở để quét khuôn mặt',
                    style:
  ...
  Widget _buildSessionCard(AttendanceSession session, int index) {
    ...
    final remainingStr =
        remaining != null ? '${remaining.inMinutes} phút' : 'Không giới hạn';
  ...
  Widget _buildEmptyState() {
    return Center(
      ...
      const Text('Không có phiên điểm danh',
          style: TextStyle(
              fontSize: 17,
              fontWeight: FontWeight.bold,
              color: AppTheme.textPrimary)),
      const SizedBox(height: 8),
      const Text('Chờ Admin mở phiên điểm danh cho lớp bạn',
...
class _StudentFaceScanScreenState extends State<StudentFaceScanScreen>
  ...
  Future<void> _processCameraImage(
    ...
    try {
      ...
      if (!_hasBlinked) {
        if (mounted)
          setState(() => _realtimeWarning = "Vui lòng chớp mắt để xác thực");
        _isProcessingFrame = false;
        return;
      }
  ...
  Future<void> _captureAndCheckin() async {
    ...
    try {
      ...
      final result = await _api.studentSelfCheckin(widget.session.id, b64,
          lat: lat, lng: lng);
      ...
      if (result['success'] == true) {
        setState(() {
          _success = true;
          _statusMessage = result['message'] ?? 'Điểm danh thành công!';
        });
        ...
        setState(() {
          _success = false;
          _statusMessage = result['message'] ?? 'Điểm danh thất bại';
        });
        Future.delayed(const Duration(seconds: 3), () {
          if (mounted)
            setState(() {
                        ...
                        child: BackdropFilter(
                          ...
                          child: Container(
                            ...
                            child: Column(
                                crossAxisAlignment: CrossAxisAlignment.start,
                                children: [
                                  Text(widget.session.tenLop,
                                      style: const TextStyle(
                                          color: Colors.white,
                                          fontWeight: FontWeight.bold,
                                          fontSize: 14)),
                                  Text(widget.session.maLop,
                                      style: TextStyle(
                                          color: Colors.white
