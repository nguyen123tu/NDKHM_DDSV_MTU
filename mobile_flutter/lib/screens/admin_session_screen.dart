...
class _AdminSessionScreenState extends State<AdminSessionScreen> {
  ...
  Future<void> _loadClasses() async {
    try { _classes = await _api.getClasses(); } catch (_) {}
  }
              ...
              child: Padding(
                ...
                child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
                  ...
                  Row(children: [
                    ...
                    const Text('Mở Phiên Điểm Danh', style: TextStyle(
                      fontSize: 20, fontWeight: FontWeight.bold, color: AppTheme.textPrimary)),
                  ]),
                  const SizedBox(height: 24),

                  // Chọn lớp
                  const Text('Chọn lớp *', style: TextStyle(fontWeight: FontWeight.w600, color: AppTheme.secondary, fontSize: 13, letterSpacing: 0.5)),
                  ...
                  // Thời lượng
                  const Text('Thời lượng', style: TextStyle(fontWeight: FontWeight.w600, color: AppTheme.secondary, fontSize: 13, letterSpacing: 0.5)),
                  const SizedBox(height: 8),
                  Row(children: [
                  ...
                  // Ghi chú
                  const Text('Ghi chú', style: TextStyle(fontWeight: FontWeight.w600, color: AppTheme.secondary, fontSize: 13, letterSpacing: 0.5)),
                  const SizedBox(height: 8),
                  TextField(
                    ...
                    child: Ink(
                      decoration: BoxDecoration(
                        gradient: selectedLopId != null
                            ? const LinearGradient(colors: [AppTheme.success, Color(0xFF059669)])
                            : null,
                        color: selectedLopId == null ? AppTheme.surfaceLight : null,
                        borderRadius: BorderRadius.circular(16),
                      ),
                      child: Container(
                        alignment: Alignment.center,
                        child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                          Icon(Icons.play_circle_fill, size: 22, color: selectedLopId != null ? Colors.white : AppTheme.textMuted),
                          const SizedBox(width: 8),
                          Text('MỞ PHIÊN ĐIỂM DANH', style: TextStyle(
                            fontWeight: FontWeight.bold, fontSize: 15, letterSpacing: 0.5,
                            color: selectedLopId != null ? Colors.white : AppTheme.textMuted,
                          )),
                        ]),
                      ),
                    ),
  ...
  Future<void> _stopSession(int sessionId) async {
    final confirm = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: const Text('Đóng phiên?', style: TextStyle(color: AppTheme.textPrimary)),
        ...
        actions: [
          ...
          TextButton(onPressed: () => Navigator.pop(ctx, true),
              child: const Text('Đóng phiên', style: TextStyle(color: AppTheme.accent))),
        ],
      ),
    );
                  ...
                  child: _isLoading
                    ...
                    : RefreshIndicator(
                        ...
                        child: _sessions.isEmpty
                          ? ListView(children: [SizedBox(height: MediaQuery.of(context).size.height * 0.2), _buildEmptyState()])
                          : ListView.builder(
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          const SizedBox(width: 12),
          const Expanded(
            child: Text('Phiên Điểm Danh', style: TextStyle(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            Container(
              ...
              child: Icon(isUrgent ? Icons.timer : Icons.class_, color: Colors.white, size: 22),
            ),
            const SizedBox(width: 14),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(session.tenLop, style: const TextStyle(
          ...
          Wrap(spacing: 8, runSpacing: 8, children: [
            ...
            _chip(Icons.timer, 'Còn $remainingStr', isUrgent ? AppTheme.warning : AppTheme.secondary),
  ...
  void _showQRDialog(AttendanceSession session) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        title: Text('QR - ${session.maLop}', textAlign: TextAlign.center,
        ...
        content: SizedBox(
          ...
          child: Column(children: [
            Container(
              ...
              child: QrImageView(data: 'MTUFACE_SESSION_${session.id}', version: QrVersions.auto, size: 180),
            ),
            const SizedBox(height: 12),
            Text('Session #${session.id}', style: const TextStyle(color: AppTheme.textMuted, fontSize: 12)),
          ]),
        ),
        actions: [
          TextButton(onPressed: () => Navigator.pop(context),
              child: const Text('Đóng', style: TextStyle(color: AppTheme.secondary))),
        ],
      ),
    );
  }
  ...
  Widget _buildEmptyState() {
    return Column(mainAxisSize: MainAxisSize.min, children: [
      ...
      const SizedBox(height: 20),
      const Text('Chưa có phiên nào đang mở', style: TextStyle(
