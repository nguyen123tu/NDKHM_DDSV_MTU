...
def validate_and_strip_exif(file_stream, max_size: int = DEFAULT_MAX_IMAGE_SIZE) -> bytes:
    """
    ...
    try:
        ...
        if len(raw_bytes) > max_size:
            ...
            raise ImageValidationError(f"Dung lượng ảnh vượt quá giới hạn cho phép ({size_mb:.1f} MB)")
        ...
        try:
            ...
            raise ImageValidationError("File tải lên không phải là hình ảnh hợp lệ hoặc bị hỏng")
        ...
        with Image.open(io.BytesIO(raw_bytes)) as img:
            ...
            output = io.BytesIO()
            save_format = img.format if img.format in ('JPEG', 'PNG', 'WEBP', 'BMP') else 'JPEG'
...
def save_validated_image(file_stream, destination_path: str, max_size: int = DEFAULT_MAX_IMAGE_SIZE) -> str:
    """
