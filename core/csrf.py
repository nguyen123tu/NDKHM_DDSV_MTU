...
def init_csrf(app):
    ...
    def csrf_protect():
        ...
        if path.startswith("/api/") or path.startswith("/chatbot/") or path.startswith("/public/api/"):
            return
        ...
        token = (
            ...
            or (request.json.get("csrf_token") if request.is_json and isinstance(request.json, dict) else None)
        )
        ...
        allowed_origins = [o.rstrip("/") for o in current_app.config.get("ALLOWED_ORIGINS", [])]
        ...
        if origin:
            ...
            if any(origin_clean == ao or origin_clean.endswith(ao) for ao in allowed_origins):
                return
        elif referer:
