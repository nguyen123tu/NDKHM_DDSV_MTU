...
def register_error_handlers(app):
    ...
    def handle_bad_request(e):
        code = 400
        message = getattr(
            e, "description", "Yêu cầu không hợp lệ hoặc thiếu dữ liệu cần thiết."
        )
    ...
    def handle_forbidden(e):
        code = 403
        message = getattr(
            e, "description", "Bạn không có quyền truy cập vào tài nguyên này."
        )
    ...
    def handle_not_found(e):
        code = 404
        message = getattr(
            e, "description", "Tài nguyên hoặc trang bạn tìm kiếm không tồn tại."
        )
    ...
    def handle_method_not_allowed(e):
        ...
        message = getattr(
            e,
            "description",
            f"Phương thức {request.method} không được hỗ trợ cho đường dẫn này.",
        )
    ...
    def handle_rate_limit(e):
        ...
        message = getattr(
            e,
            "description",
            "Bạn đang gửi quá nhiều yêu cầu. Vui lòng thử lại sau giây lát.",
        )
        logger.warning(
            f"429 Too Many Requests: {request.method} {request.path} - {request.remote_addr}"
        )
        if _is_api_request():
            return jsonify({"success": False, "error": code, "message": message}), code
        return (
    ...
    def handle_unhandled_exception(e):
        ...
        if isinstance(e, HTTPException):
            ...
            logger.warning(
                f"HTTPException {code}: {request.method} {request.path} - {message}"
            )
            if _is_api_request():
                return (
