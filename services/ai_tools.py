...
def execute_tool(tool_call):
    ...
    if func_name == "search_knowledge":
        ...
        return _tool_export_report(args.get("ma_lop", ""), args.get("format", "excel"), args.get("date"))
...
def _tool_export_report(ma_lop: str, fmt: str, date: str = None) -> str:
    ...
    lop = execute_one("SELECT id, ma_lop FROM lop_hoc WHERE ma_lop LIKE %s OR ten_lop LIKE %s", (f"%{ma_lop}%", f"%{ma_lop}%"))
    if not lop:
