...
def backfill():
    ...
    try:
        ...
        res1 = execute_update("UPDATE diem_danh SET status = 'PRESENT' WHERE trang_thai = 'Co mat' AND (status IS NULL OR status = '')")
        print(f"Updated {res1} legacy 'Co mat' records to status = 'PRESENT'.")
        
        # Some records might just have status = NULL entirely
        res2 = execute_update("UPDATE diem_danh SET status = 'PRESENT', trang_thai = 'Co mat' WHERE status IS NULL")
        if res2 > 0:
