...
def run_migration():
    ...
    commands = [
        ...
        BEGIN
            ...
            ALTER TABLE diem_danh ADD CONSTRAINT fk_diem_danh_phien FOREIGN KEY (phien_id) REFERENCES phien_diem_danh(id) ON DELETE NO ACTION;
        END;
        """,
        """
        IF NOT EXISTS (SELECT * FROM sys.indexes WHERE name = 'uq_phien_sinh_vien' AND object_id = OBJECT_ID('diem_danh'))
        BEGIN
            CREATE UNIQUE INDEX uq_phien_sinh_vien ON diem_danh(phien_id, sinh_vien_id) WHERE phien_id IS NOT NULL AND sinh_vien_id IS NOT NULL;
        END;
        """,
        """
    ...
    for i, cmd in enumerate(commands, 1):
        try:
            execute_update(cmd)
            print(f"  [{i}/{len(commands)}] Success.")
        except Exception as e:
            print(f"  [{i}/{len(commands)}] Info/Error: {e}")
