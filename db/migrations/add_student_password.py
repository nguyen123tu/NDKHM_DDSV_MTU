...
def run_migration():
    ...
    try:
        sql_alter = (
            "ALTER TABLE sinh_vien ADD COLUMN password_hash VARCHAR(255) AFTER mssv;"
        )
    ...
    students = execute_query(
        "SELECT id, mssv FROM sinh_vien WHERE password_hash IS NULL"
    )
    ...
    for sv in students:
        ...
        execute_update(
            "UPDATE sinh_vien SET password_hash = %s WHERE id = %s", (pwd_hash, sv_id)
        )
        count += 1
