...
def get_classes():
    ...
    try:
        ...
        res_data = [
            {"id": c["id"], "ma_lop": c["ma_lop"], "ten_lop": c["ten_lop"]}
            for c in classes
        ]
...
def get_schedule():
    ...
    if payload.get("role") != "student":
        return (
            jsonify({"success": False, "message": "Chỉ sinh viên mới có lịch học"}),
            403,
        )
    ...
    if not student or not student["lop_id"]:
        return (
            jsonify(
                {"success": True, "data": [], "message": "Sinh viên chưa được xếp lớp"}
            ),
            200,
        )
...
def mark_all_notifications_read():
    ...
    try:
        ...
        user_id = payload.get("sub")
        execute_update(
            "UPDATE thong_bao SET da_doc = 1 WHERE sinh_vien_id = %s", (user_id,)
        )
...
def mobile_chatbot_ask():
    ...
    if not question:
        return (
            jsonify({"success": False, "message": "Câu hỏi không được để trống"}),
            400,
        )
    ...
    if len(question) > 2000:
        return (
            jsonify(
                {"success": False, "message": "Câu hỏi quá dài (tối đa 2000 ký tự)"}
            ),
            400,
        )
