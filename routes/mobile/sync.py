...
def push_attendance():
    ...
    for log in logs:
        ...
        try:
            # Tìm sinh viên
            sv = execute_one(
                "SELECT id, lop_id FROM sinh_vien WHERE mssv = %s", (mssv,)
            )
            if not sv:
                errors.append(
                    {"local_uuid": local_uuid, "error": "Sinh viên không tồn tại"}
                )
                continue
                ...
                errors.append(
                    {
                        ...
                        "error": "Không tìm thấy phiên điểm danh phù hợp",
                    }
                )
                continue
            ...
            if result and (
                result.get("success")
                or result.get("action") == "observed"
                or result.get("error_code") == "DUPLICATE_EVENT"
            ):
    ...
    return (
        jsonify(
            {"success": True, "synced_local_uuids": synced_uuids, "errors": errors}
        ),
        200,
    )
...
def sync_schedule():
            ...
            jsonify(
                {
                    "success": True,
                    "data": [],
                    "message": "Chỉ sinh viên mới có lịch học",
                }
            ),
            200,
        )
    ...
    try:
        sql = (
            "SELECT * FROM lich_hoc WHERE lop_id = %s ORDER BY thu ASC, gio_bat_dau ASC"
        )
