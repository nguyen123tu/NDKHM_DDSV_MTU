...
def mobile_checkin():
    ...
    if role == 'student' and (not nonce or not timestamp_ms):
        return jsonify({"success": False, "message": "Thiếu thông tin chữ ký xác thực (nonce/timestamp)"}), 400
    ...
    if role == 'student':
        if lat is None or lng is None:
            return jsonify({"success": False, "message": "Thiếu vị trí GPS khi điểm danh"}), 400
    ...
    if not log_result or not log_result.get('success'):
        error_msg = log_result.get('message', 'Check-in bị bỏ qua') if log_result else 'Check-in thất bại'
        error_code = log_result.get('error_code', 'UNKNOWN') if log_result else 'UNKNOWN'
...
def get_stats():
    ...
    try:
        if payload.get('role') == 'student':
            ...
            absent_sv = total_sessions - present_sv - late_sv if total_sessions > (present_sv + late_sv) else 0
            attendance_rate = ((present_sv + late_sv) / total_sessions * 100) if total_sessions > 0 else 0
...
def delete_attendance_record(record_id):
    ...
    try:
        ...
        if auth_error:
            return auth_error
        if payload.get('role') == 'student':
            return jsonify({"success": False, "message": "Chỉ Admin mới được hủy bản ghi"}), 403

        data = request.get_json(silent=True) or {}
        reason = (data.get('reason') or '').strip()
        if not reason:
            return jsonify({"success": False, "message": "Bắt buộc nhập lý do khi hủy bản ghi"}), 400
...
def student_self_checkin():
    ...
    if session_row.get('vi_do') is not None and session_row.get('kinh_do') is not None:
        ...
        distance = calculate_distance(sv_lat, sv_lng, session_row['vi_do'], session_row['kinh_do'])
        max_radius = 50
    ...
    if not log_result or not log_result.get('success'):
        ...
        return jsonify({
            "success": False,
            "message": log_result.get('message', 'Điểm danh thất bại') if log_result else 'Điểm danh thất bại'
        }), 409
...
def mobile_leave_request():
    ...
    if not lop_id or not session_id or not ly_do:
        return jsonify({"success": False, "message": "Thiếu thông tin lớp, phiên điểm danh hoặc lý do"}), 400
        
    from db.connection import execute_one, execute_query
    # Kiểm tra phiên
    session_row = execute_one("SELECT * FROM phien_diem_danh WHERE id = %s AND lop_id = %s", (session_id, lop_id))
    if not session_row:
        return jsonify({"success": False, "message": "Phiên điểm danh không tồn tại cho lớp này"}), 404
        
    # Kiểm tra nếu phiên đã đóng và quá hạn xin phép
    if session_row['trang_thai'] == 0:
        return jsonify({"success": False, "message": "Phiên điểm danh đã đóng, không thể gửi đơn xin phép nữa"}), 403
        
    # Kiểm tra xem sinh viên có thuộc lớp này không
    sv = execute_one("SELECT id FROM sinh_vien WHERE id = %s AND lop_id = %s", (sinh_vien_id, lop_id))
    if not sv:
        return jsonify({"success": False, "message": "Bạn không thuộc lớp này"}), 403
        
    # Kiểm tra xem đã gửi đơn cho phiên này chưa
    existing = execute_one("SELECT id FROM don_xin_phep WHERE sinh_vien_id = %s AND phien_id = %s", (sinh_vien_id, session_id))
    if existing:
        return jsonify({"success": False, "message": "Bạn đã gửi đơn xin phép cho phiên này rồi"}), 409
