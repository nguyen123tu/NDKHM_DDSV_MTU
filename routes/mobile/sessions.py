...
def create_session():
    ...
    return jsonify({
        ...
        "data": {
            ...
            "ma_lop": lop['ma_lop'],
            "ten_lop": lop['ten_lop'],
            "het_han": session_info.get('het_han', datetime.now() + timedelta(minutes=duration_minutes)).strftime('%Y-%m-%d %H:%M:%S') if hasattr(session_info.get('het_han'), 'strftime') else str(session_info.get('het_han')),
...
def stop_session_api(session_id):
    ...
    from services.attendance_session_service import AttendanceSessionService
    result = AttendanceSessionService.close_session(session_id, admin_id=payload.get('sub'))
    ...
    return jsonify({"success": True, "message": "Đã đóng phiên điểm danh và tổng hợp báo cáo", "data": result}), 200
...
def get_session_details(session_id):
    ...
    try:
        ...
        bat_dau_str = session_row['bat_dau'].strftime('%Y-%m-%d %H:%M:%S') if session_row.get('bat_dau') and hasattr(session_row['bat_dau'], 'strftime') else str(session_row.get('bat_dau'))
...
def delete_session(session_id):
    ...
    try:
        ...
        if auth_error:
            return auth_error
        if payload.get('role') == 'student':
            return jsonify({"success": False, "message": "Chỉ Admin mới được hủy phiên"}), 403

        data = request.get_json(silent=True) or {}
        reason = (data.get('reason') or '').strip() or 'Admin hủy phiên từ mobile'

        from services.attendance_session_service import AttendanceSessionService
        result = AttendanceSessionService.cancel_session(session_id, admin_id=payload.get('sub'), reason=reason)
