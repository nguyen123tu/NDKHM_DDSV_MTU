...
def api_lookup():
    ...
    total_attendance = (
        stats_query["total_attendance"]
        if stats_query and stats_query["total_attendance"]
        else 0
    )
    total_present = (
        stats_query["total_present"]
        if stats_query and stats_query["total_present"]
        else 0
    )
    ...
    if not avatar_path or not os.path.exists(
        os.path.join(Config.BASE_DIR, str(avatar_path))
    ):
        if face_images:
    ...
    return jsonify(
        {
            ...
            "student": {
                ...
                "gioi_tinh": (
                    "Nam"
                    if sv.get("gioi_tinh") == 1
                    else ("Nữ" if sv.get("gioi_tinh") == 0 else "Chưa cập nhật")
                ),
                "avatar": avatar_path,
            },
            "stats": {"total": total_attendance, "present": int(total_present)},
...
def selfcheck():
    ...
    classes = execute_query(
        "SELECT id, ma_lop, ten_lop FROM lop_hoc WHERE trang_thai = 1 ORDER BY ma_lop"
    )
...
def _do_recognize(image_data):
    ...
    if len(face_results) == 0:
        return {
            "success": False,
            "msg": "Không phát hiện khuôn mặt. Hãy nhìn thẳng vào camera.",
        }
        ...
        if sv_temp:
            execute_update(
                ...
                (
                    sv_temp["id"],
                    "Spoofing",
                    f"Phát hiện dùng ảnh giả: {spoof_reason}. Độ trùng khớp: {sim:.2f}",
                    evidence_path,
                ),
            )
        return {
    ...
    if sv.get("is_locked") == 1:
        return {
            "success": False,
            "msg": "Tài khoản của bạn đã bị khóa do vi phạm quy chế. Vui lòng liên hệ Admin.",
        }
...
def api_recognize():
    ...
    if rtsp_url:
        ...
        if frame is None:
            return jsonify(
                {"success": False, "msg": "Không lấy được khung hình từ Camera IP"}
            )
        ...
        data["image"] = "data:image/jpeg;base64," + base64.b64encode(buffer).decode(
            "utf-8"
        )
    ...
    if not lop_id:
        return jsonify(
            {"success": False, "msg": "Vui lòng chọn lớp học trước khi điểm danh!"}
        )
