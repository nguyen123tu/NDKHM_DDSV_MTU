...
def index():
    ...
    return render_template(
        "dashboard/support.html", requests=requests, active_page="support"
    )
...
def resolve(req_id):
    if not session.get("admin_id"):
        return (
            jsonify({"success": False, "msg": "Unauthorized: Vui lòng đăng nhập"}),
            401,
        )
    if session.get("admin_role") != "admin":
        return (
            jsonify(
                {"success": False, "msg": "Forbidden: Yêu cầu quyền Quản trị (Admin)"}
            ),
            403,
        )
    ...
    try:
        execute_update(
            "UPDATE yeu_cau_ho_tro SET trang_thai = 'Đã giải quyết' WHERE id = %s",
            (req_id,),
        )
