...
def cancel_attendance(alert_id):
    ...
    log = execute_one("SELECT sinh_vien_id, thoi_gian FROM gian_lan_log WHERE id = %s", (alert_id,))
    if not log or not log['sinh_vien_id']:
        return jsonify({'success': False, 'message': 'Không tìm thấy thông tin gian lận hoặc sinh viên không xác định'}), 404
    ...
    if res != -1:
        execute_update("UPDATE gian_lan_log SET da_xu_ly = 1 WHERE id = %s", (alert_id,))
        return jsonify({'success': True, 'message': f'Đã hủy toàn bộ điểm danh ngày {date_str}'})
...
def lock_account(alert_id):
    ...
    log = execute_one("SELECT sinh_vien_id FROM gian_lan_log WHERE id = %s", (alert_id,))
    if not log or not log['sinh_vien_id']:
    ...
    if res != -1:
        ...
        return jsonify({'success': True, 'message': f'Tài khoản {status_msg}', 'is_locked': new_status})
